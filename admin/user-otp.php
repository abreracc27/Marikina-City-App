...
<body class="body-margin marikina-color">
    ...
    <div class="login-form container-fluid">
		<form action="user-otp" method="POST" autocomplete="off">
			<div class="row">
                <div class="col-12 col-lg-12 order-lg-first">
                    <span class="login-text">Account Verification</span>
                </div>
                <?php if (isset($_SESSION['success_message']) && !empty($_SESSION['success_message'])) { ?>
                    <div class="col-12 col-lg-12 mt-1">
                        ...
                        <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="<?php echo $_SESSION['icon']; ?>:"><use xlink:href="#<?php echo $_SESSION['icon-type']; ?>-fill"/></svg>
				...
				<?php } ?>
				<div class="col-12 col-lg-12 mt-3">
                    <input class="login-button" type="submit" name="check" value="Submit">
				</div>
				<div class="col-12 col-lg-12 mt-3">
					<span class="create-account">Didn't receive the verification OTP? <a style="text-decoration: none;" id="cd" href="controller?resend=1&page=user-otp">Resend<span id="timer"><span id="time"></span></span></a></span>
				</div>
            </div>
        </div>
    </div>
...
<script>
	...
	setTimeout(function() {
		button.href="controller?resend=1&page=user-otp"; 
		button.style="text-decoration: none;";      
	}, counter * 1000);

	$(document).on("click", "#cd", function(event){
		window.location.replace("controller?resend=1&page=user-otp");
		if($('#time').length > 0){
			if(sessionStorage.getItem("counter")){
				if(sessionStorage.getItem("counter") > 0){
					var counter = sessionStorage.getItem("counter");
